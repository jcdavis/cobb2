...
dline* dline_upsert(dline* existing,
  ...
  if(existing == NULL) {
    ...
    memcpy(((char*)new_entry)+sizeof(dline_entry), string+start,
           new_str_len);
    ...
    *dline_upsert_mode = DLINE_UPSERT_MODE_INSERT;
     ...
     * dline to see if the suffix exists, and if so what it's existing score
     * is. Further suffix upserts will know which operation to perform,
     * which saves us some string comparisons.
     */
    ...
    int insert_offset = 0;
    int is_last_insert = 0;
  
    while(current->score > score) {
      if((uint64_t)(current->global_ptr)&1) {
        is_last_insert=1;
        break;
      }
      current += (sizeof(dline_entry) + current->len +
                  wasted_buffer(current->len))>>3;
    }*/
